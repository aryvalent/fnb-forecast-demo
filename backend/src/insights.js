...
export function buildInsights({
  ...
  for (const r of dishForecastTotals) {
    ...
    totalByOutlet.set(
      r.outlet,
      (totalByOutlet.get(r.outlet) ?? 0) + Number(r.predicted_quantity ?? 0)
    );
  }
  ...
  if (topDish) {
    ...
    insights.push(
      `${dish} demand is expected to ${dir} by ${Math.abs(delta).toFixed(0)}% vs last 7 days`
    );
  }
  ...
  if (topOutlet) {
    ...
    insights.push(
      `${outlet} is forecasted to have the highest demand (${Math.round(qty)} dish units)`
    );
  }
