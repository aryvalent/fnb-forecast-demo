...
function avgFromStats(stats, dow, { minPointsPerDow, minTotalPoints }) {
  ...
  if (stats && stats.overallCount >= minTotalPoints) {
    return {
      value: stats.overallSum / stats.overallCount,
      source: 'series_overall',
    };
  }
  return null;
}
...
function pickWithFallback({
  ...
  const seriesPick = avgFromStats(seriesStats, dow, {
    minPointsPerDow,
    minTotalPoints,
  });
  if (seriesPick) return seriesPick;

  const dishPick = avgFromStats(dishStats.get(dish), dow, {
    minPointsPerDow,
    minTotalPoints,
  });
  if (dishPick)
    return {
      ...dishPick,
      source: dishPick.source === 'series_dow' ? 'dish_dow' : 'dish_overall',
    };

  const outletPick = avgFromStats(outletStats.get(outlet), dow, {
    minPointsPerDow,
    minTotalPoints,
  });
  if (outletPick)
    return {
      ...outletPick,
      source: outletPick.source === 'series_dow' ? 'outlet_dow' : 'outlet_overall',
    };

  const globalPick = avgFromStats(globalStats, dow, {
    minPointsPerDow,
    minTotalPoints,
  });
  if (globalPick)
    return {
      ...globalPick,
      source: globalPick.source === 'series_dow' ? 'global_dow' : 'global_overall',
    };
...
async function buildBaselineStats(pool, { startDate, lookbackDays = 28, historyDays = 90 }) {
  ...
  const seriesUniverse = await getSeriesUniverse(pool, {
    historyStart,
    historyEnd,
  });
  ...
  const globalOverall = globalStats.overallCount
    ? globalStats.overallSum / globalStats.overallCount
    : 0;
...
export async function forecastBaselineDaily(
  ...
  const ctx = await buildBaselineStats(pool, {
    startDate,
    lookbackDays,
    historyDays,
  });
...
export function forecastTotalsFromAverages(averages, { startDate, days = 7 }) {
  ...
  for (const s of averages) {
    ...
    out.push({
      outlet: s.outlet,
      dish: s.dish,
      predicted_quantity: Math.round(total),
    });
  }
...
export async function computeBacktestMape(pool, { testDays = 14, lookbackDays = 28 } = {}) {
  ...
  for (const [, m] of seriesMap.entries()) {
    for (let i = 0; i < testDays; i++) {
      ...
      // lookbackEnd = subDays(d, 1) (unused for now)
      const dow = d.getDay();
...
export async function computeBacktestKpis(pool, { testDays = 14, lookbackDays = 28 } = {}) {
  ...
  for (let i = 0; i < testDays; i++) {
    dailyTotals.push({
      date: toIsoDate(addDays(testStart, i)),
      actual: 0,
      model: 0,
      naive: 0,
    });
