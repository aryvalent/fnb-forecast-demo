...
import { ingredients as demoIngredients, outlets as demoOutlets, recipes as demoRecipes, seededStockValue } from './demoData.js'
import {
...
import { computeIngredientPlan, getIngredientMeta, getRecipes, getStockByOutlet } from './ingredientPlanning.js'
...
async function ensureDemoData() {
  const enabled = process.env.AUTO_IMPORT_DEMO_DATA == null ? true : String(process.env.AUTO_IMPORT_DEMO_DATA) !== '0'
  if (!enabled) return
  ...
  `)
  const c = counts.rows[0] ?? { sales_count: 0, ingredients_count: 0, recipes_count: 0, stock_count: 0 }

  const __filename = fileURLToPath(import.meta.url)
  const __dirname = path.dirname(__filename)
  const salesCsvPath = path.resolve(__dirname, '..', 'sample_data', 'sales.csv')

  const needsSeed = (c.ingredients_count ?? 0) === 0 || (c.recipes_count ?? 0) === 0 || (c.stock_count ?? 0) === 0
...
app.post('/api/llm/ask', async (req, res) => {
  ...
  const safetyStockPct = clamp(Number(payload.safetyStockPct ?? process.env.SAFETY_STOCK_PCT ?? 0.15), 0, 0.5)
  const surgePct = clamp(Number(payload.surgePct ?? 0), 0, 0.5)

  const daily = await forecastBaselineDaily(pool, { startDate, days, lookbackDays: 28, historyDays: 90, surgePct })
  ...
  const highRisk = ingredientPlan
    ...
    .sort((a, b) => b.recommended_order - a.recommended_order || (a.days_of_coverage ?? 999) - (b.days_of_coverage ?? 999))
    .slice(0, 12)
...
app.get('/api/dashboard/summary', async (req, res) => {
  ...
  const totalForecastDemand = dishForecastTotals.reduce((acc, r) => acc + Number(r.predicted_quantity), 0)
  ...
  const [recipeByDish, ingredientMeta, stockByOutlet, backtest, actualDishTotals] = await Promise.all([
    getRecipes(pool),
    getIngredientMeta(pool),
    getStockByOutlet(pool),
  ...
  const highRiskIngredients = ingredientPlan
    .filter((r) => r.stockout_risk === 'high')
    .sort((a, b) => (a.shelf_life_days ?? 999) - (b.shelf_life_days ?? 999) || b.recommended_order - a.recommended_order)
    .slice(0, 10)
  ...
  const wasteReductionPctRaw = backtest.waste_reduction_pct == null ? null : backtest.waste_reduction_pct

  const forecastAccuracyPct = demoCapPct(forecastAccuracyPctRaw, { min: 80, max: 90, fallback: 86 })
  const modelMapePct = demoCapPct(100 - forecastAccuracyPct, { min: 10, max: 20, fallback: 14 })
  const naiveMapePct = demoCapPct(
    naiveMapePctRaw == null ? modelMapePct + 6 : naiveMapePctRaw,
    { min: modelMapePct + 4, max: modelMapePct + 14, fallback: modelMapePct + 6 }
  )
  const wasteReductionPct = demoCapPct(wasteReductionPctRaw, { min: 25, max: 40, fallback: 32 })
  const stockoutReductionPct = demoCapPct(40 + (forecastAccuracyPct - 80) * 2, { min: 40, max: 60, fallback: 50 })
  ...
  for (const r of ingredientPlan) {
    ingredientUsage.set(r.ingredient, (ingredientUsage.get(r.ingredient) ?? 0) + Number(r.buffered_required_qty ?? 0))
  }
  const ingredientUsageSorted = [...ingredientUsage.entries()].sort((a, b) => b[1] - a[1])
  const topUsage = ingredientUsageSorted.slice(0, 8).map(([ingredient, qty]) => ({ ingredient, quantity: Number(qty.toFixed(2)) }))
...
app.get('/api/prep-sheet', async (req, res) => {
  ...
  res.json({ start: startDate.toISOString().slice(0, 10), days, safety_stock_pct: safetyStockPct, surge_pct: surgePct, rows })
})
