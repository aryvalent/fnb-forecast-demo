...
export async function getIngredientMeta(pool) {
	const { rows } = await pool.query(
		"select ingredient, shelf_life_days, unit from ingredients",
	);
	const map = new Map();
	for (const r of rows) {
...
export function computeIngredientPlan({
	...
	for (const f of dishForecastTotals) {
		...
		for (const r of recipes) {
			...
			prev.required_qty +=
				Number(f.predicted_quantity) * Number(r.qty_per_dish);
			acc.set(key, prev);
		}
	}
	...
	for (const v of acc.values()) {
		...
		const maxStockByShelfLife =
			meta.shelf_life_days != null &&
			Number.isFinite(meta.shelf_life_days)
		...
		out.push({
			...
			shelf_life_capped:
				maxStockByShelfLife != null &&
				bufferedRequired > maxStockByShelfLife,
		});
	}
